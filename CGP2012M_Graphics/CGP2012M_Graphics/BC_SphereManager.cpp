...
void BC_SphereManager::update(GLuint elapsedTime, Camera cam, float bX_r, float bX_l, float bY_t, float bY_b, BC_Player &player, std::vector<BC_Bullet> &bullets)
{
	for (int i = 0; i < amount; i++) {
		...
		if (pEuclidianDistance <= 0.9) {
			...
			float bEuclidianDistance = std::sqrt(pow((bubbles[i].position.x - bullets[0].position.x), 2.0f) + pow((bubbles[i].position.y - bullets[0].position.y), 2.0f) + pow((bubbles[i].position.z - bullets[0].position.z), 2.0f));
			if (bEuclidianDistance <= 0.6) {
				bullets[0].collided = true;
				bubbles.erase(bubbles.begin() + i);
				this->amount--;
			}
			else {
